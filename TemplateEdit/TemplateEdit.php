...
class TemplateEdit extends SpecialPage {
	...
	static public function onBaseTemplateToolbox( BaseTemplate $skinTemplate, array &$toolbox  ) {
		...
		if($title!=null) {
			...
			if($title!=null) {
				...
				if($title->exists()) {
					...
					$source=ContentHandler::getContentText($wikipage->getRevision()->getContent());
					...
					$content=new WikitextContent( $source );
					$article->doEditContent( $content,wfMessage('templateedit-templatecreated',$this->templatename)->text());
				}
			};
	...
	function showStartingForm() {
		...
		if ($this->templatename!="")
			$wgOut->addWikiTextAsContent(wfMessage('templateedit-createwithtemplate',$this->templatename)->text());
		$wgOut->addHTML(
			Xml::openElement( 'form', array( 'id' => 'templateedit', 'action' => $this->getPageTitle()->getFullUrl(), 'method' => 'post' ) ) .
	...
	function showSelectTemplateForm($title,$source) {
		...
		$wgOut->addWikiTextAsContent('[[:'.$title->getPrefixedText().']]');
		$wgOut->addHTML(
			Xml::openElement( 'form', array( 'id' => 'templateedit', 'action' => $this->getPageTitle()->getFullUrl(), 'method' => 'post' ) ) .
			html::hidden( 'title',  $this->getPageTitle()->getPrefixedText() ) .
	...
	function getTemplateDefinitions($template) {
		...
		return Array(wfMessage('templateedit-noeditor')->text()=>Array('type'=>'TITLE','must'=>FALSE,'description'=>'','picklist'=>'', 'default'=>'', 'migration'=>''));
	}
	...
	function addInput($wgOut,$name,$def,$value,$style) {
		...
		$wgOut->addWikiTextAsContent(trim($must.$def['description']));
		...
		if(($def['type']=='TEXT')||($def['type']=='COORDS')||($def['type']=='LINK')||($def['type']=='LINKNONS')||($def['type']=='NUMBER')) {
			$newvalue=$value;
			if($def['type']=='COORDS') {
				$newvalue=preg_replace('%([0-9\-]*)\s*;\s*([0-9\-]*)\s*;\s*%','$1 $2, ',$value);
...
//		print_r($templateparts);
		...
		$wgOut->addWikiTextAsContent(wfMessage('templateedit-editformintro',$title->getPrefixedText(),$templateparts['template'])->text());
		$wgOut->addHTML(
			Xml::openElement( 'form', array( 'id' => 'templateedit', 'action' => $this->getPageTitle()->getFullUrl(), 'method' => 'post' ) ) .
	...
	function showSavedMessage($title,$source,$article) {
		...
		$content=new WikitextContent( $newsource );
		$article->doEditContent( $content,wfMessage('templateedit-templateedited',$templateparts['template'])->text());
		$wgOut->addWikiTextAsContent('[[:'.$title->getPrefixedText().']]');
		$wgOut->addHTML(
